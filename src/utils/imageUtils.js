...
export const getFirstImage = (images) => {
  ...
  // Get the first image path and convert it, then optimize for device
  const processedPath = getImagePath(images[0]);
...
export const getOptimizedImageUrl = (url) => {
  ...
  const cdnPattern = /^https:\/\/imagedelivery\.net\/[^\/]+\/[^\/]+\/(public|mob)(?:$|[?#])/i;
  if (cdnPattern.test(normalizedUrl)) {
    const isMobile = isMobileDevice();
    return normalizedUrl.replace(/\/(public|mob)(?=$|[?#])/i, isMobile ? '/mob' : '/public');
  }
